...
g= datetime.datetime(2023, 4, 5, 00, 00 ,0)
...
print(f'''
  ...
  {G}👨‍💻 PY_09
         ...
         {B} اتحدى اي واحد يسوي مثل اداتي ''') 

print('')
print('')
token=input(f"{Y}Enter Token : {G}")
print('')
ID=input(f"{Y}Enter ID  : {G}")
print('\n')
print(G+'---------------------------------------------------------')
print(f'                {Y}  بــــــسم الـــلــه الرحــمــٰـن الرحيم')
print(G+'---------------------------------------------------------')
def az():
 while True:
  ...
  useq=str("".join(random.choice(user)for i in range(rng)))
  ...
  try:
   while True:
        mn += 1
        user=(rr['users'][mn]['user']['username'])
...
👨‍💻 @PY_09      '''
        	       ...
        	       print(G+ff)
        	       requests.post(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text="+str(ff))
        	else:
        		print(X+f' [×] ERROR Gmail : {email}')
        else:
        	print(E+f' [×] ERROR : {email}')
  except IndexError:
  	az()
  except KeyError:
  	az()
